...
/** Offers a direct workspace edit that extracts the selected expression into a local `val`. */
internal object LSKotlinExtractVariableCodeActionProvider : LSCodeActionProvider {
    ...
    override fun getCodeActions(params: CodeActionParams): Flow<CodeAction> = flow {
        val action = server.withAnalysisContext {
            readAction {
                ...
                val extraction = ExtractVariableComputation.extract(
                    psiFile,
                    TextRange(
                ...
                val insertionPosition = TextRange(extraction.declarationOffset, extraction.declarationOffset)
                    .toLspRange(document).start
                val edits = listOf(
                    TextEdit(Range(insertionPosition, insertionPosition), extraction.declaration),
                    TextEdit(extraction.expressionRange.toLspRange(document), extraction.variableName),
                )
                CodeAction(
