...
/** Replaces a call to an expression-bodied function with that expression. */
internal object LSKotlinInlineFunctionCodeActionProvider : LSCodeActionProvider {
    ...
    override fun getCodeActions(params: CodeActionParams): Flow<CodeAction> = flow {
        val action = server.withAnalysisContext {
            readAction {
                ...
                val inlining = InlineFunctionComputation.inlineAt(
                    psiFile,
                    document.offsetByPosition(params.range.start),
                ) ?: return@readAction null

                CodeAction(
                    title = "Inline call to '${inlining.functionName}'",
                    kind = CodeActionKind.RefactorInline,
                    diagnostics = null,
                    isPreferred = null,
                    disabled = null,
                    edit = WorkspaceEdit(changes = mapOf(params.textDocument.uri to listOf(
                        TextEdit(inlining.callRange.toLspRange(document), inlining.replacement),
                    ))),
                    command = null,
                    data = null,
                )
