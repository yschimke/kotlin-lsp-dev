...
/** Offers a direct workspace edit that extracts the selected statements into a private function. */
internal object LSKotlinExtractFunctionCodeActionProvider : LSCodeActionProvider {
    ...
    override fun getCodeActions(params: CodeActionParams): Flow<CodeAction> = flow {
        val action = server.withAnalysisContext {
            readAction {
                ...
                val extraction = ExtractFunctionComputation.extract(
                    psiFile,
                    TextRange(
                ...
                val insertAt = TextRange(extraction.functionInsertOffset, extraction.functionInsertOffset)
                    .toLspRange(document).start
                val edits = listOf(
                    TextEdit(extraction.selectionRange.toLspRange(document), extraction.callText),
                    TextEdit(Range(insertAt, insertAt), extraction.functionText),
                )
                val signature = if (extraction.parameters.isEmpty()) {
                    "no parameters"
                } else {
                    extraction.parameters.joinToString(", ")
                }
                CodeAction(
                    title = "Extract function '${extraction.functionName}' ($signature)",
